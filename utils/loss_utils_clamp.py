...
def get_loss_clamp(pcds_pred, gt, partial, cfg=None):
    """loss function
    Args
    ...
    cdc = chamfer_distance(Pc, gt_c, norm=cfg.TRAIN.CD_NORM)[0]
    cd1 = chamfer_distance(P1, gt_1, norm=cfg.TRAIN.CD_NORM)[0]
    cd2 = chamfer_distance(P2, gt_2, norm=cfg.TRAIN.CD_NORM)[0]
    cd3 = chamfer_distance(P3, gt_3, norm=cfg.TRAIN.CD_NORM)[0]

    loss_decoder = cdc + cd1 + cd2 + cd3

    partial_matching = chamfer_distance(
        partial, P1, norm=cfg.TRAIN.CD_NORM, single_directional=True
    )[
        0
    ...
    return loss_total, losses, [gt, gt_c, gt_c]
